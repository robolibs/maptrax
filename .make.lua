-- maptrax's build, as recipes. This replaced the Makefile; there is no other.
--
...
make.recipe{
  ...
  run = function()
    sh.maturin("build", "--features", "python-extension,geojson")
    local wheel = oslo.fs.glob("target/wheels/" .. NAME .. "-*.whl")[1]
    assert(wheel, "no wheel found in target/wheels")
    sh.pip("install", "--quiet", "--no-deps", "--upgrade", "--target", "target/python", wheel)
  end,
}
