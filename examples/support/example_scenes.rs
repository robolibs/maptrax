...
pub fn centered_obstacle(border: &Polygon, half_size: f64) -> Polygon {
    ...
    rect_obstacle(center_x, center_y, half_size, half_size)
}

pub fn rect_obstacle(center_x: f64, center_y: f64, half_w: f64, half_h: f64) -> Polygon {
...
pub fn offset_multi_obstacles(border: &Polygon) -> Vec<Polygon> {
    let vertices = border.exterior().points().collect::<Vec<_>>();
    let (min_x, max_x) = vertices
        .iter()
        .fold((f64::INFINITY, f64::NEG_INFINITY), |acc, p| {
