//! Reverse-capable machine (Reeds-Shepp): turns can back up, so they fit in a
//! shallow headland band. Two machines split the field; turns route through the
//! densely-interpolated headland nodes and stay inside the field.
//!
//! Companion: `turn_radius_no_reverse.rs` (forward-only Dubins).
//!
//! Run a Rerun viewer (or `rerun --serve`) then:
//!   cargo run --example turn_radius_reverse
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let planned = planner.plan_machines_for_part(
        ...
        RoutingOptions { strategy: RoutingStrategy::Snake, local_improvement_passes: 0 },
        &cfg,
    )?;
    ...
    println!("=== Reverse-capable (Reeds-Shepp), {HEADLANDS} headlands, swath {SWATH_WIDTH} m ===\n");
